  ...
  const handleKeyDown = (e)=>{
    ...
    if(e.key === "Enter"){
        if(activeIndex >= 0){
            ...
            setQuery(selected.title || selected.city || selected.address);
            handleSearch(selected.title || selected.city || selected.address);
        }else{
            handleSearch();
        }
    }
        ...
        <div className="absolute top-[70px] left-4 w-[762px] bg-white shadow-lg rounded-xl z-40 border">
            {suggestions.map((item,index) =>(
                <div 
                    ...
                    className={`p-3 cursor-pointer hover: rounded-xl bg-white ${
